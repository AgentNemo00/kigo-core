...
type InquiryRenderPayload struct {
	...
	FPS 			uint 			// FPS choosen for the video stream, e.g. 30, 60, must be bigger than 0
	Time 			time.Duration 	// Duration; FPS * Time = Animation length; optional zero value means until ringbuffer closes	

	MaxFrameSize 	uint32 			// max frame size, i.e. custom size, FullHD, 4K, in pixel. If zero, width*height is used. if it exceed screens maximum, it is croped.
	Timeout 		time.Duration 	// Duration to context timeout on receiver site while waiting for new data. Max should be 1sec/FPS

	ObjectID        uint32 			// uint32 id of the object to manipulate which already exist, if none given a new one is generated
}	
